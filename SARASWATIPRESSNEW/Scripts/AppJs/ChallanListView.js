...
$(function () {
    $('#btnView').click(function () {
        ...
        $.ajax({
            url: "/InvoiceCumChallanReqList/GetChallanViewData",
            type: 'POST',
                ...
                $.each(data, function (i, item) {
                   ...
                   // items += "<option value='" + item.Value + "'>" + item.Text + "</option>";
                    ...
                    if (item.IsInvoiceCreated == "0") {
                        HtmlItems += "      <a href='/InvoiceCumChallanReqList/PrintOperation/" + item.ChallanId + "?Command=Print' target='_blank'>Print</a>&nbsp;|&nbsp;";
                        HtmlItems += "      <a href='/InvoiceCumChallanReqList/EditOperation/" + item.ChallanId + "?Command=Edit'>Edit</a>";
                    }
                    else {
                        HtmlItems += "      <a href='/InvoiceCumChallanReqList/PrintOperation/" + item.ChallanId + "?Command=Print' target='_blank'>Print</a>";                        
                    }
...
function SendSms() {
    ...
    $.ajax({
        url: "/InvoiceCumChallanReqList/SMSSend",
        type: 'POST',
...
function SendEmail() {
    ...
    $.ajax({
        url: "/InvoiceCumChallanReqList/SendEmail",
        type: 'POST',
        data: { griddata: _griddata },
        cache: false,
        success: function (data) {
